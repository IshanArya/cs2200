...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        vpn_t entryVpn = frame_table[victim_pfn].vpn;
        pcb_t *frameProcess = frame_table[victim_pfn].process;
        pte_t *ptEntry =
            (pte_t *)(&mem[frameProcess->saved_ptbr * PAGE_SIZE]) + entryVpn;
        if (ptEntry->dirty) {
            swap_write(ptEntry, mem + (victim_pfn * PAGE_SIZE));
            ptEntry->dirty = FALSE;
            stats.writebacks++;
        }
        ptEntry->valid = FALSE;
        frame_table[victim_pfn].mapped = FALSE;
    }
...
/*  --------------------------------- PROBLEM 9
   -------------------------------------- Checkout PDF section 7, 9, and 11 for
   this problem
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        timestamp_t leastRecent = UINT32_MAX;

        for (pfn_t i = 1; i < num_entries; i++) {
            fte_t ftEntry = frame_table[i];
            if (!ftEntry.protected && ftEntry.timestamp < leastRecent) {
                last_unprotected = i;
                leastRecent = ftEntry.timestamp;
            }
        }

        // printf("Last Unprotected: %d\n", last_unprotected);
        // printf("Least Recent time: %d\n", leastRecent);

        if (leastRecent < UINT32_MAX) {
            return last_unprotected;
        }
    }
