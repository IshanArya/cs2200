...
/*  --------------------------------- PROBLEM 1
   -------------------------------------- Checkout PDF Section 2 For this
   Problem

    Split the virtual address into VPN and offset.
    These macros will be used whenever you get a memory address from the CPU and
...
static inline int getFrameNumber(void* b) {
    return ((uint8_t*)b - mem) / PAGE_SIZE;
}
