...
static packet_t *packetize(char *buffer, int length, int *count) {
    ...
    int numberOfPackets = length / MAX_PAYLOAD_LENGTH;
    numberOfPackets += length % MAX_PAYLOAD_LENGTH > 0 ? 1 : 0;

    *count = numberOfPackets;

    packets = malloc(sizeof(packet_t) * (size_t)numberOfPackets);
    ...
    for (int i = 0; i < numberOfPackets; i++) {
        if (i < numberOfPackets - 1) {
            ...
            packets[i].payload_length = length % MAX_PAYLOAD_LENGTH > 0
                                            ? length % MAX_PAYLOAD_LENGTH
                                            : MAX_PAYLOAD_LENGTH;
            packets[i].type = LAST_DATA;
        }

        memcpy(packets[i].payload, &buffer[i * MAX_PAYLOAD_LENGTH],
               packets[i].payload_length);
        packets[i].checksum =
            checksum(packets[i].payload, packets[i].payload_length);
    }
...
static void *rtp_recv_thread(void *void_ptr) {
    ...
    do {
        ...
        do {
            if (net_recv_packet(connection->net_connection_handle, &packet) <=
                    0 ||
                packet.type == TERM) {
            ...
            if (packet.type == DATA || packet.type == LAST_DATA) {
                if (packet.checksum !=
                    checksum(packet.payload, packet.payload_length)) {
                    packet.type = DATA;
                    resPacket.type = NACK;
                } else {
                    buffer = realloc(buffer, (size_t)(buffer_length +
                                                      packet.payload_length));
                    memcpy(buffer + buffer_length, packet.payload,
                           packet.payload_length);
                    buffer_length += packet.payload_length;
                    resPacket.type = ACK;
                }
                net_send_packet(connection->net_connection_handle, &resPacket);
            }
        ...
        if (connection->alive == 1) {
            ...
            pthread_cond_broadcast(&connection->recv_cond);
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        packet_array =
            packetize(message->buffer, message->length, &array_length);
        ...
        for (i = 0; i < array_length; i++) {
            ...
            pthread_mutex_lock(&connection->ack_mutex);
            while (!connection->responded) {
                pthread_cond_wait(&connection->ack_cond,
                                  &connection->ack_mutex);
            }

            connection->responded = 0;
            if (!connection->acknowledged) i--;
            pthread_mutex_unlock(&connection->ack_mutex);
        }
...
rtp_connection_t *rtp_connect(char *host, int port) {
    ...
    if ((net_connection_handle = net_connect(host, port)) < 1) return NULL;
...
int rtp_recv_message(rtp_connection_t *connection, char **buffer, int *length) {
    ...
    if (connection->alive == 0) return -1;
    /* lock */
    pthread_mutex_lock(&connection->recv_mutex);
